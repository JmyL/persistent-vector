// https://github.com/CharlesFrasch/cppcon2023/blob/main/Fifo4a.hpp
//
#include <atomic>
#include <cassert>
#include <memory>
...
/// Threadsafe, efficient circular FIFO with cached cursors; bitwise AND vs
/// remainder
...
public:
    ...
    explicit LFQueue(size_type capacity, Alloc const& alloc = Alloc{})
        : Alloc{alloc}, mask_{capacity - 1},
    ...
    /// @return `true` if the pop operation is successful; `false` if fifo is
    /// empty.
    auto pop(T& value)
    {
        auto popCursor = popCursor_.load(std::memory_order_relaxed);
...
private:
    ...
    // See Fifo3 for reason std::hardware_destructive_interference_size is not
    // used directly
    ...
    alignas(hardware_destructive_interference_size) size_type
        popCursorCached_{};
